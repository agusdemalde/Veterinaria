...
class MascotaController {
	...
	def clinicaService
		...
		def addVisita() {
			...
			def visita = clinicaService.crearVisita((params.visitaMascotaId ?: 0) as Long, params.visita_descripcion, params.visita?.fecha)
			if (visita.hasErrors()) {
		...
		def find() {
			...
			def mascotas = Mascota.findAllByNombre(params.nombre)
			if (!mascotas) {
				return [message: 'mascota.not.found']
			}
	
			
				render view: 'selection', model: [mascotas: mascotas]
